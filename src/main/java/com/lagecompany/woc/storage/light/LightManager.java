...
public class LightManager {
	...
	public static void setup(Are areInstance) {
		are = areInstance;
	}
	...
	public static void computeSmoothLighting(Chunk chunk, VoxelReference voxel) {
		...
		for (int side = Voxel.FRONT; side <= Voxel.DOWN; side++) {
			...
			case Voxel.FRONT: {
				...
				data.setVertexData(side, 0, computeSmoorthLightingVertex(sideLighting, neighborhood[15],
						neighborhood[10], neighborhood[17])); // v0
				data.setVertexData(side, 1, computeSmoorthLightingVertex(sideLighting, neighborhood[15],
						neighborhood[8], neighborhood[12])); // v1
				data.setVertexData(side, 2,
						computeSmoorthLightingVertex(sideLighting, neighborhood[3], neighborhood[8], neighborhood[0])); // v2
				data.setVertexData(side, 3,
						computeSmoorthLightingVertex(sideLighting, neighborhood[3], neighborhood[10], neighborhood[5])); // v3
				break;
			}
			case Voxel.RIGHT: {
				// The right face is composed of v1, v5, v6 and v2.
				data.setVertexData(side, 0, computeSmoorthLightingVertex(sideLighting, neighborhood[13],
						neighborhood[8], neighborhood[12])); // v1
				data.setVertexData(side, 1, computeSmoorthLightingVertex(sideLighting, neighborhood[13],
						neighborhood[9], neighborhood[14])); // v5
				data.setVertexData(side, 2,
						computeSmoorthLightingVertex(sideLighting, neighborhood[1], neighborhood[9], neighborhood[2])); // v6
				data.setVertexData(side, 3,
						computeSmoorthLightingVertex(sideLighting, neighborhood[1], neighborhood[8], neighborhood[0])); // v2
				break;
			}
			case Voxel.BACK: {
				// The back face is composed of v5, v4, v7 and v6.
				data.setVertexData(side, 0, computeSmoorthLightingVertex(sideLighting, neighborhood[16],
						neighborhood[9], neighborhood[14])); // v5
				data.setVertexData(side, 1, computeSmoorthLightingVertex(sideLighting, neighborhood[16],
						neighborhood[11], neighborhood[19])); // v4
				data.setVertexData(side, 2,
						computeSmoorthLightingVertex(sideLighting, neighborhood[4], neighborhood[11], neighborhood[7])); // v7
				data.setVertexData(side, 3,
						computeSmoorthLightingVertex(sideLighting, neighborhood[4], neighborhood[9], neighborhood[2])); // v6
				break;
			}
			case Voxel.LEFT: {
				// The left face is composed of v4, v0, v3 and v7.
				data.setVertexData(side, 0, computeSmoorthLightingVertex(sideLighting, neighborhood[18],
						neighborhood[11], neighborhood[19])); // v4
				data.setVertexData(side, 1, computeSmoorthLightingVertex(sideLighting, neighborhood[18],
						neighborhood[10], neighborhood[17])); // v0
				data.setVertexData(side, 2,
						computeSmoorthLightingVertex(sideLighting, neighborhood[6], neighborhood[10], neighborhood[5])); // v3
				data.setVertexData(side, 3,
						computeSmoorthLightingVertex(sideLighting, neighborhood[6], neighborhood[11], neighborhood[7])); // v7
				break;
			}
			case Voxel.TOP: {
				// The top face is composed of v3, v2, v6 and v7.
				data.setVertexData(side, 0,
						computeSmoorthLightingVertex(sideLighting, neighborhood[3], neighborhood[6], neighborhood[5])); // v3
				data.setVertexData(side, 1,
						computeSmoorthLightingVertex(sideLighting, neighborhood[3], neighborhood[1], neighborhood[0])); // v2
				data.setVertexData(side, 2,
						computeSmoorthLightingVertex(sideLighting, neighborhood[4], neighborhood[1], neighborhood[2])); // v6
				data.setVertexData(side, 3,
						computeSmoorthLightingVertex(sideLighting, neighborhood[4], neighborhood[6], neighborhood[7])); // v7
				break;
			}
			case Voxel.DOWN: {
				// The down face is composed of v4, v5, v1 and v0.
				data.setVertexData(side, 0, computeSmoorthLightingVertex(sideLighting, neighborhood[16],
						neighborhood[18], neighborhood[19])); // v4
				data.setVertexData(side, 1, computeSmoorthLightingVertex(sideLighting, neighborhood[16],
						neighborhood[13], neighborhood[14])); // v5
				data.setVertexData(side, 2, computeSmoorthLightingVertex(sideLighting, neighborhood[15],
						neighborhood[13], neighborhood[12])); // v1
				data.setVertexData(side, 3, computeSmoorthLightingVertex(sideLighting, neighborhood[15],
						neighborhood[18], neighborhood[17])); // v0
				break;
			}
			default: {
	...
	private static float computeSmoorthLightingVertex(int sideLighting, byte side1, byte side2, byte corner) {
		// if side1 and side2 are both opaque or have no light, corner should be darker also, becouse side1 and side2
